...
const generateAscensionChartStatistics = (heroList: Array<Hero>): Array<StatisticChart> => {
  ...
  chartList.push(new StatisticChart('Progress (copies)', StatisticChartType.RADIAL, progressChartData));
  return chartList;
};
  ...
  heroList.forEach((hero: Hero) => {
    ...
    if (awakened) {
      if (ascendedNeededCopies > 0) {
        ascendedInfo.totalAwakenedCopiesNeeded += ascendedNeededCopies;
        ascendedInfo.copiesAwakenedNeeded.push({ amount: ascendedNeededCopies, hero });
      }
